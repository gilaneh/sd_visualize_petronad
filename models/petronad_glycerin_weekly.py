...
class SdVisualizePetronadCalculate(models.Model):
    ...
    def calculate(self, function_name, diagram_id, update_date):
        ...
        try:
            if function_name == 'petronad_glycerin_weekly':
                res['value'] = self.petronad_glycerin_weekly(diagram_id, update_date)
    ...
    def petronad_glycerin_weekly(self, diagram=0, update_date=0):
        ...
        #                              and rec.register_type == 'feed_usage']))
        week_productions_list = {'day_glycerin_pitch': [], 'day_glycerin_i': [], 'day_glycerin_p': [], }
        ...
        for rec_date in date_week_day:
            ...
            week_productions_list['day_glycerin_i'].append(day_glycerin_i)
            week_productions_list['day_glycerin_p'].append(day_glycerin_p)

        week_sum_glycerin_pitch = sum(week_productions_list['day_glycerin_pitch'])
        week_sum_glycerin_i = sum(week_productions_list['day_glycerin_i'])
        week_sum_glycerin_p = sum(week_productions_list['day_glycerin_p'])
